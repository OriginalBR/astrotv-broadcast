...
type MobileTab = 'scoreboard' | 'lowerThirds' | 'transitions' | 'ticker' | 'master';
...
export const MobileControlDeck: React.FC<MobileControlDeckProps> = ({ onSwitchToDesktop }) => {
  ...
  const handleResetScore = () => {
    if (currentSb) {
      updateScoreboard(currentSb.id, {
        teamA: { ...currentSb.teamA, score: 0 },
        teamB: { ...currentSb.teamB, score: 0 },
      });
    }
  };
  ...
  return (
    <div className="h-[100dvh] w-screen bg-[#070a12] text-white flex flex-col overflow-hidden select-none touch-manipulation font-sans">
      ...
      {/* 2. Main Responsive Stage (Retrato vs Paisagem) */}
      <div className={`flex-1 flex overflow-hidden ${isLandscape ? 'flex-row' : 'flex-col'}`}>
        
        {/* PGM Live Monitor Preview Box */}
        <div className={`${
          isLandscape 
            ? 'w-[45%] h-full border-r border-white/10 p-2 flex flex-col justify-between' 
            : 'h-[32%] w-full p-2.5 flex-shrink-0'
        }`}>
          <div className="relative w-full h-full bg-black rounded-lg border-2 border-red-500/60 overflow-hidden shadow-xl flex items-center justify-center">
            {/* Live On-Air Pill */}
            <div className="absolute top-1.5 left-2 z-40 flex items-center gap-1.5 bg-black/80 backdrop-blur px-2 py-0.5 rounded text-[10px] font-black uppercase font-condensed">
              ...
              <span className={isAnyOnAir ? 'text-red-400' : 'text-slate-400'}>
                {isAnyOnAir ? 'NO AR' : 'STANDBY'}
              </span>
            </div>

            {/* Program Output Render Miniature */}
            <div 
              className="w-full h-full pointer-events-none transform origin-top-left"
              style={{
                width: '1920px',
                height: '1080px',
                transform: `scale(${isLandscape ? 0.22 : 0.18})`,
              }}
            >
              <RenderOverlay
        ...
        <div className={`flex-1 flex flex-col justify-between p-2.5 overflow-hidden ${isLandscape ? 'w-[55%]' : 'w-full'}`}>
          
          {/* TAB 1: PLACAR AO VIVO (SCOREBOARD PAD) */}
          {activeTab === 'scoreboard' && currentSb && (
            <div className="flex-1 flex flex-col justify-between gap-2 h-full">
              ...
              <div className="grid grid-cols-2 gap-2 flex-1 items-stretch">
                {/* Team A */}
                <div className="bg-[#0e1422] border border-white/10 rounded-xl p-2.5 flex flex-col justify-between shadow-lg">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      <span className="w-2.5 h-4 rounded-xs shadow" style={{ backgroundColor: currentSb.teamA.color }} />
                      <span className="font-black text-sm text-white font-condensed uppercase truncate max-w-[90px]">
                        {currentSb.teamA.shortName || currentSb.teamA.name}
                      </span>
                    </div>
                    <button
                      onClick={() => incrementScore('teamA', -1)}
                      className="w-7 h-7 bg-slate-800 active:bg-slate-700 text-slate-300 rounded flex items-center justify-center text-xs font-bold"
                    >
                      <Minus className="w-3.5 h-3.5" />
                    </button>
                  </div>

                  {/* Huge Score +1 Touch Area */}
                  <button
                    onClick={() => incrementScore('teamA', 1)}
                  ...
                  >
                    ...
                    <span className="text-[10px] font-black uppercase text-slate-400 mt-1 tracking-wider">
                      +1 PONTO / GOL
                    </span>
                  </button>
                </div>
              ...
              <div className="flex items-center gap-2 h-14 flex-shrink-0">
                {/* Timer Play/Pause Big Button */}
                <button
                  onClick={toggleScoreboardTimer}
                  className={`flex-1 h-full rounded-xl flex items-center justify-center gap-2 font-black text-base uppercase font-condensed shadow-lg border transition-all ${
                ...
                >
                  ...
                  <span className="font-mono text-xl">{timeFormatted}</span>
                  <span className="text-xs">{currentSb.matchTime.isRunning ? 'PAUSAR' : 'INICIAR'}</span>
                </button>

                {/* Reset Score / Reset Clock */}
                <button
                  onClick={handleResetTimer}
                  title="Zerar Cronômetro"
                  className="w-12 h-full bg-[#111726] active:bg-slate-700 rounded-xl border border-white/10 flex items-center justify-center text-slate-300"
                >
                  <RotateCcw className="w-4 h-4" />
                </button>

                {/* Placar On-Air Master Toggle */}
                <button
                  onClick={() => setScoreboardOnAir(isSbOnAir ? null : currentSb.id)}
                  className={`px-4 h-full rounded-xl flex items-center justify-center gap-1.5 font-black text-xs uppercase font-condensed shadow-lg transition-all ${
                    isSbOnAir
          ...
          {/* TAB 2: TARJAS (LOWER THIRDS QUICK PAD) */}
          ...
          {/* TAB 3: VINHETAS & TRANSIÇÕES (STINGER MPC PADS) */}
          {activeTab === 'transitions' && (
            <div className="flex-1 grid grid-cols-2 gap-2 h-full items-stretch">
              ...
              >
                <Sparkles className="w-7 h-7 text-yellow-300 mb-1" />
              ...
              >
                <Tv className="w-7 h-7 text-cyan-200 mb-1" />
              ...
              >
                <Radio className="w-7 h-7 text-pink-200 mb-1" />
              ...
              >
                <Sparkles className="w-7 h-7 text-emerald-200 mb-1" />
            ...
            <div className="flex-1 flex flex-col justify-between gap-2 h-full">
              ...
              {onSwitchToDesktop && (
                ...
                >
                  ...
                  <span>Alternar para Visão Completa de Desktop</span>
                </button>
              )}
            </div>
          )}
      ...
      {/* 3. Fixed Native Bottom Navigation Bar (Zero Scroll / High-Impact Touch) */}
      <nav className="h-16 bg-[#090d16] border-t border-white/10 grid grid-cols-4 px-1 z-30 flex-shrink-0">
        {/* Tab 1: Placar */}
        <button
          onClick={() => setActiveTab('scoreboard')}
          className={`flex flex-col items-center justify-center gap-1 transition-all ${
            activeTab === 'scoreboard' ? 'text-amber-400 bg-white/5' : 'text-slate-400 active:text-white'
          }`}
        >
          <Trophy className="w-5 h-5" />
          <span className="text-[10px] font-black uppercase tracking-wider font-condensed">PLACAR</span>
        </button>
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center gap-1 transition-all ${
            activeTab === 'lowerThirds' ? 'text-red-400 bg-white/5' : 'text-slate-400 active:text-white'
          }`}
        >
          <User className="w-5 h-5" />
          <span className="text-[10px] font-black uppercase tracking-wider font-condensed">TARJAS GC</span>
        </button>
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center gap-1 transition-all ${
            activeTab === 'transitions' ? 'text-yellow-400 bg-white/5' : 'text-slate-400 active:text-white'
          }`}
        >
          <Sparkles className="w-5 h-5" />
          <span className="text-[10px] font-black uppercase tracking-wider font-condensed">VINHETAS</span>
        </button>
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center gap-1 transition-all ${
            activeTab === 'ticker' ? 'text-blue-400 bg-white/5' : 'text-slate-400 active:text-white'
          }`}
        >
          <Sliders className="w-5 h-5" />
          <span className="text-[10px] font-black uppercase tracking-wider font-condensed">TICKER/LOGO</span>
        </button>
      </nav>
    </div>
  );
